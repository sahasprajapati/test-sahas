...
interface Props {
    ...
    title: string;
    name: string;
    handleScroll?: (e: React.UIEvent<HTMLDivElement>, startLoading: () => void, stopLoading: () => void) => Promise<void>;
...
const MultiSelectComponent: React.FC<Props> = (props) => {
    ...
    React.useEffect(() => {
        ...
        if (typeof props.Options === 'function') {
            setLoading(true);
            props.Options().then((selectOptions) => {
                setOptions(selectOptions);
                if (typeof props.getAllOptions === 'function')
                    props.getAllOptions(selectOptions)
            ...
            if (typeof props.getAllOptions === 'function')
                props.getAllOptions(options)
        }


        if (typeof props.SelectedValues === 'function') {
            setLoading(true);
            props.SelectedValues().then((selectValues) => {
    ...
    const toggleOption = (item: any) => {
        ...
        if (!props.isSignalSelect) {

            const selectedIndex: any = selected.find((element: any) => item[props.getIdProperty] === element[props.getIdProperty]);
            if (selectedIndex) {
                updatedSelected = selected.filter((i: any) => selectedIndex[props.getIdProperty] !== i[props.getIdProperty]);
            } else {
    ...
    const filterOptionsBySearch = (searchTerm: string) => {
        ...
        if (searchTerm === '') {
            ...
            if (props.isMultiOptional) {
                ...
                updatedOptions = options.filter((item: any) => item.name.toLowerCase().includes(lowerCaseSearchTerm));
            ...
            if (!itemsFound) {
                ...
                setLoadingLoad(true)
                if (typeof props.Options === 'function')
                    props.Options({ keyword: searchTerm }).then((response) => {
                ...
                setFilteredOptions(updatedOptions);  // Update the options displayed
        }
    };
            ...
            return (
                <div className="relative">
                    ...
                    <div className="divide-y dark:divide-slate-200/5">
                        {loadingLoad && getItems.length < 1 ? <div className="loading"></div> :
                            getItems.map((item: any, index: number) => {
                                item.selected = Boolean(selected.find((e: any) => e[props.optionLabelProperty] == item[props.optionLabelProperty]))
                                const isSelected = item.selected && <OkIcon />
                                return (
                                    <div key={item[props.getIdProperty]} className="cursor-pointer w-full hover:bg-gray-100" onClick={(e) => {
                                        toggleOption(item)
                                    }}>
                                        <div className="flex w-full items-center p-2 pl-2  relative">
                                            {loadingLoad && getItems.length == index + 1 ? <div className="loading"></div> :
                                                <div className="flex w-full justify-between	items-center gap-4 p-4">
                                                    <strong className="text-slate-900 text-sm font-medium dark:text-slate-200">{item[props.optionLabelProperty]}</strong>
                                                    {isSelected}
                                                </div>
                                            }
                                        </div>
    ...
    const renderOptions = () => {
        const displayOptions = searchInput ? filteredOptions : options;
        return displayOptions.map((option: any) => {
            option.selected = Boolean(selected.find((e: any) => e[props.optionLabelProperty] == option[props.optionLabelProperty]))
            return (
                <div key={option[props.getIdProperty]} className="cursor-pointer w-full border-gray-100 rounded-t border-b hover:bg-gray-100" onClick={() => {
                    toggleOption(option)
                }}>
        ...
        <div ref={dropdownRef} className="w-full min-h-14 flex flex-col items-center mx-auto">
            ...
            <div className="inline-block relative w-full">
                ...
                <div className="flex flex-col items-center relative">
                            ...
                            <div className="flex flex-auto flex-wrap gap-x-1 cursor-pointer">
                                {loading ? <div className="loading"></div> :
                                    selected.map((element: any, index) => (
                                        <div key={element[props.getIdProperty]} className="flex justify-center items-center m-1 font-medium py-1 px-1 bg-white rounded bg-gray-100 border">
                                            <div style={{ minWidth: `${props.minWidth}px` }}
                                                className="text-xs cursor-default font-normal leading-none max-w-full flex-initial">
                                                {element[props.optionLabelProperty]}
                                            </div>
                                            <div className="flex flex-auto flex-row-reverse" onClick={(event) => {
                    ...
                    {isOpen &&
                        //@ts-ignore
                        <div style={{ top: `${(heightRef?.current?.clientHeight as number + 23) ?? 65}px` }}
                                    ...
                                    <div
                                        onScroll={(e) => {
                                            if (props.handleScroll && !loadingLoad)
                                                props.handleScroll(e, startLoadingLoad, stopLoadingLoad).then((selectOptions: any) => {
